...
# Load your dataset (from your own processed RData file)
load("~/Desktop/ShinyDashVax-main/data/immunodata.RData")
...
```{r}
ui <- fluidPage(
  titlePanel("Michigan School Immunization Dashboard (District Level)"),

  sidebarLayout(
    sidebarPanel(
      selectInput("grade", "Select Grade Level", 
                  choices = c("Kindergarten" = "K", "7th Grade" = "7"),
                  selected = "K"),
      selectInput("county", "Select County", 
                  choices = sort(unique(dfDistrict$COUNTY)))
    ),

    mainPanel(
      tabsetPanel(
        tabPanel("District Data",
                 h4("Vaccine Waiver Rates by District"),
                 DTOutput("district_table"),
                 plotOutput("district_plot", height = 600)
        )
      )
    )
  )
...
server <- function(input, output, session) {
  ...
  # Reactive variable to pick the correct waiver column
  waiver_col <- reactive({
    paste0(input$grade, "_pctWaiver")
  })

  # Create filtered data for the selected county
  district_data <- reactive({
    dfDistrict %>%
      filter(COUNTY == input$county) %>%
      mutate(WaiverRate = .data[[waiver_col()]])
  })
    ...
    datatable(
      district_data() %>%
        transmute(
          District = DISTRICT,
          County = COUNTY,
          `Waiver Rate (%)` = round(WaiverRate, 1)
        ) %>%
  ...
  output$district_plot <- renderPlot({
    ggplot(district_data(), aes(x = reorder(DISTRICT, WaiverRate), y = WaiverRate)) +
      geom_col(fill = "#3182bd") +
      coord_flip() +
      labs(
        title = paste("Vaccine Waiver Rates by District –", input$county),
        subtitle = paste(ifelse(input$grade == "K", "Kindergarten", "7th Grade")),
        x = "District",
        y = "Waiver Rate (%)"
      ) +
...
```
```{r}
shiny::runApp("/Users/ashnapatel/Desktop/ShinyDashVax-main")

```

```{r}
tabPanel("District Dashboard",
         sidebarLayout(
           sidebarPanel(
             selectInput("district_grade", "Select Grade Level",
                         choices = c("Kindergarten" = "K", "7th Grade" = "7"),
                         selected = "K"),
             selectInput("district_county", "Select County",
                         choices = sort(unique(dfDistrict$COUNTY))),
             helpText("Select a county and grade to view district-level waiver rates.")
           ),
           mainPanel(
             tabsetPanel(
               tabPanel("District Table",
                        DTOutput("district_table")),
               tabPanel("District Plot",
                        plotOutput("district_plot", height = 600))
             )
           )
         ))
...
server <- function(input, output, session) {
  ...
  # ---- County stuff ----
  # your county server code here...

  # ---- District stuff ----
  district_waiver_col <- reactive({
    paste0(input$district_grade, "_pctWaiver")
  })
  ...
  district_data <- reactive({
    ...
    dfDistrict %>%
      ...
      mutate(WaiverRate = .data[[district_waiver_col()]])
  })
  ...
  output$district_plot <- renderPlot({
    ggplot(district_data(), aes(x = reorder(DISTRICT, WaiverRate), y = WaiverRate)) +
      ...
      coord_flip() +
      labs(
        title = paste("Vaccine Waiver Rates by District –", input$district_county),
        subtitle = paste(ifelse(input$district_grade == "K", "Kindergarten", "7th Grade")),
        x = "District",
        y = "Waiver Rate (%)"
      ) +
      theme_minimal(base_size = 14)
